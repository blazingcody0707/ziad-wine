...
static struct image_file_map* image_locate_build_id_target(const BYTE* id, unsigned idlen)
{
    ...
    if (!(fmap_link = HeapAlloc(GetProcessHeap(), 0, sizeof(*fmap_link))))
        return NULL;
    if (image_locate_build_id_target_in_dir(fmap_link, id, idlen, L"/data/data/app.gamenative/files/usr/lib/debug/.build-id/"))
        return fmap_link;
    if (image_locate_build_id_target_in_dir(fmap_link, id, idlen, L"/data/data/app.gamenative/files/usr/lib/.build-id/"))
        return fmap_link;
            ...
            if (fmap_link)
            {
                ...
                if (!ret)
                {
                    dst = HeapAlloc(GetProcessHeap(), 0,
                                    sizeof(L"/data/data/app.gamenative/files/usr/lib/debug/.build-id/") + (3 + filename_len + idlen * 2) * sizeof(WCHAR));
                    if (dst)
                    {
                        WCHAR* p;
                         ...
                         * so try both
                         */
                        p = memcpy(dst, L"/data/data/app.gamenative/files/usr/lib/debug/.build-id/", sizeof(L"/data/data/app.gamenative/files/usr/lib/debug/.build-id/"));
                        p += wcslen(dst);
